  ...
  useEffect(() => {
    const fetchTokenAndPostDetails = async () => {
      try {
        ...
        const response = await API.get(endpoints["post-detail"](postId), {
  ...
  const CommentItem = ({ comment, onReply, depth = 0 }) => {
    ...
    return (
      <View style={[styles.comment]}>
        <Image
          source={{ uri: comment.user.avatar.replace(/^image\/upload\//, "") }}
        ...
        <View>
          <View style={styles.commentContainer}>
              ...
              <View>
                ...
                {editImage && (
                  <View style={styles.selectedImageContainer}>
                    <Image
                      source={{ uri: getValidImageUrl(editImage) }}
            ...
            {comment.image && !isEditing && (
              <Image
                source={{ uri: getValidImageUrl(comment.image) }}
            ...
            {!commentsLocked && (
              <View style={styles.replyAndSeeMoreContainer}>
                {depth < MAX_REPLY_DEPTH && (
                  <TouchableOpacity
                    onPress={() => changeReplyingTo(comment.id)}
                  >
                ...
                {comment.replies?.length > 0 && (
                  <TouchableOpacity onPress={() => handleSeeMore(comment.id)}>
                    <Text style={styles.seeMoreButton}>
                      {showReplies[comment.id] ? "Ẩn bớt" : "Xem thêm"}
                    </Text>
                  </TouchableOpacity>
                )}
              </View>
            ...
            <View style={styles.post}>
              ...
              <View style={{ flexDirection: "column", flex: 1 }}>
                {(user.role === 0 || user.id === post.user.id) && (
                  <TouchableOpacity
                    onPress={handleToggleCommentsLock}
                    style={{ marginLeft: "auto" }}
                  >
                    <Ionicons
                      name={commentsLocked ? "lock-closed" : "lock-open"}
                      size={24}
                      color="red"
                    />
                  </TouchableOpacity>
